...
static CFRunLoopModeRef __CFRunLoopFindMode(CFRunLoopRef rl, CFStringRef modeName, Boolean create) {
    ...
    srlm._name = modeName;
    // 根据给定的Value 获取想要的值 Obtains a specified value from a set.
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
	...
	__CFPortSet waitSet = rlm->_portSet;
        ...
        //2、通知observer 即将处理 Timer 事件
        if (rlm->_observerMask & kCFRunLoopBeforeTimers) __CFRunLoopDoObservers(rl, rlm, kCFRunLoopBeforeTimers);
            //3、通知observer 即将处理 source0 事件
        if (rlm->_observerMask & kCFRunLoopBeforeSources) __CFRunLoopDoObservers(rl, rlm, kCFRunLoopBeforeSources);
        //RunLoop 的存放 BLock 的任务
...
/**
 CommonMode : 标记！！！！
CFRunLoopAddTimer([NSRunLoop currentRunLoop](main), _timer, CommonMode)
 */
void CFRunLoopAddTimer(CFRunLoopRef rl, CFRunLoopTimerRef rlt, CFStringRef modeName) {    
    ...
    if (modeName == kCFRunLoopCommonModes) {
    /**
     <CFSetRef>{defaultMode, TrackingMode}
     */
	CFSetRef set = rl->_commonModes ? CFSetCreateCopy(kCFAllocatorSystemDefault, rl->_commonModes) : NULL;
    /**
      <CFSetRef>{observer, source,.....}
     */
	if (NULL == rl->_commonModeItems) {
	    rl->_commonModeItems = CFSetCreateMutable(kCFAllocatorSystemDefault, 0, &kCFTypeSetCallBacks);
	}
    /**
     <CFSetRef>{observer, source,.....，timer(自己创建的！！！)}
     */
	CFSetAddValue(rl->_commonModeItems, rlt);
	if (NULL != set) {
	    CFTypeRef context[2] = {rl, rlt};
	    /* add new item to all common-modes */
        //timer <defaultMode  TrackingMode>,Timer 的事件源
	    CFSetApplyFunction(set, (__CFRunLoopAddItemToCommonModes), (void *)context);
	    CFRelease(set);
	}
    } else {
        //defalutMode CFRunLoopModeRef
	CFRunLoopModeRef rlm = __CFRunLoopFindMode(rl, modeName, true);
	if (NULL != rlm) {
            if (NULL == rlm->_timers) {
